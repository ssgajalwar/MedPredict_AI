...
class LightGBMForecaster:
    """
    LightGBM-based forecasting model for hospital demand prediction
    """
    ...
    def load_data(self):
        ...
        print("=" * 60)
        print("LIGHTGBM FORECASTER - DATA LOADING")
        print("=" * 60)
    ...
    def train(self, target_column='total_patients', lag_days=30, 
        ...
        """
        print("\n" + "=" * 60)
        print("LIGHTGBM FORECASTER - MODEL TRAINING")
        print("=" * 60)
    ...
    def forecast(self, horizon_days=7):
        ...
        """
        print("\n" + "=" * 60)
        print(f"LIGHTGBM FORECASTER - GENERATING {horizon_days}-DAY FORECAST")
        print("=" * 60)
        ...
        for i, future_date in enumerate(forecast_dates):
            ...
            # This is a simplified approach - in production, you'd update all features
        ...
        for col in self.feature_columns:
            if col not in future_features.columns:
                if col in data.columns:
                    ...
                    # If feature doesn't exist, set to 0
    ...
    def visualize_results(self, forecast_results, output_dir='./results'):
        ...
        """
        print("\n" + "=" * 60)
        print("LIGHTGBM FORECASTER - GENERATING VISUALIZATIONS")
        print("=" * 60)
        ...
        plot_history_and_forecast(
            ...
            save_path=f"{output_dir}/lightgbm_forecast_{len(forecast_results['forecast_dates'])}day.png",
            show_plot=False
        )
        ...
        if hasattr(self, 'y_test'):
            ...
            plot_actual_vs_predicted(
                ...
                self.target_column,
                model_name="LightGBM",
                save_path=f"{output_dir}/lightgbm_actual_vs_predicted.png",
                show_plot=False
            )
        ...
        plot_feature_importance(
            ...
            top_n=15,
            model_name="LightGBM",
            save_path=f"{output_dir}/lightgbm_feature_importance.png",
            show_plot=False
        )
        ...
        if hasattr(self, 'y_test'):
            ...
            plot_residuals(
                ...
                self.target_column,
                model_name="LightGBM",
                save_path=f"{output_dir}/lightgbm_residuals.png",
                show_plot=False
            )
        
        print(f"\n+ All visualizations saved to: {output_dir}/")
    
    def save_forecast_to_csv(self, forecast_results, output_path='lightgbm_forecast.csv'):
        """
...
def run_lightgbm_forecast(target_column='total_patients', horizon_days=7, 
                          data_dir='../Data_Generator/hospital_data_csv',
                          output_dir='./results'):
    """
    Complete LightGBM forecasting pipeline
    ...
    """
    print("=" * 60)
    print("LIGHTGBM HOSPITAL DEMAND FORECASTING")
    print("=" * 60)
    
    # Initialize forecaster
    forecaster = LightGBMForecaster(data_dir)
    ...
    forecaster.save_forecast_to_csv(
        forecast_results, 
        output_path=f"{output_dir}/lightgbm_forecast_{horizon_days}day.csv"
    )
    
    print("\n" + "=" * 60)
    print("✓ LIGHTGBM FORECASTING COMPLETED SUCCESSFULLY!".replace("✓", "+"))
    print("=" * 60)
